...
# --- كلاس أزرار التنقل (للتنقل بين الصفحات) ---
class TopView(discord.ui.View):
    def __init__(self, sorted_data, interaction):
        ...
        self.page = 0
        self.per_page = 10 # عرض 10 في كل صفحة
    ...
    def get_embed(self):
        ...
        # شكل الإمبد "نوفا" (بدون fields، الاعتماد على description فقط)
        embed = discord.Embed(
            title="💬 أفضل نقاط الكتابة", 
            color=0x2F3136 # لون غامق يشبه ديسكورد
        )
        ...
        for i, (user_id, info) in enumerate(page_data, start=start + 1):
            ...
            # حساب مستوى تقريبي (إذا لم يكن لديك نظام مستوى خاص)
            xp = info['xp']
            level = xp // 100 
            
            # أيقونات الترتيب (توزيع الأيقونات مثل نوفا)
            rank_icon = "🔹" if i == 1 else "🔸" if i == 2 else "🔸" 
            
            # التنسيق: #1 | الاسم - خبرة: X | مستوى: Y
            description += f"#{i} | **{display_name}** — خبرة: `{xp}` | مستوى: `{level}`\n\n"
        
        embed.description = description if description else "لا توجد بيانات حالياً."
        embed.set_footer(text=f"صفحة {self.page + 1}")
        return embed
...
class Stats(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if user_id not in data: data[user_id] = {"name": message.author.name, "xp": 0}
        data[user_id]["xp"] += 1
        self.save_data(data)
    ...
    async def top(self, interaction: discord.Interaction):
        data = self.load_data()
        if not data:
            return await interaction.response.send_message("❌ لا توجد بيانات.", ephemeral=True)
