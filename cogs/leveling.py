...
async def create_rank_card(member: discord.Member, level: int, xp: int, xp_needed: int, role_name: str):
    ...
    draw.rounded_rectangle([bar_x, bar_y, bar_x + bar_w, bar_y + bar_h], radius=16, fill=(50, 55, 65, 255))
    
    if percent > 0:
        filled_w = int(bar_w * percent)
        draw.rounded_rectangle([bar_x, bar_y, bar_x + filled_w, bar_y + bar_h], radius=16, fill=(0, 229, 255, 255))

    # كتابة تفاصيل الـ XP والمستوى بجانب الشريط
    xp_text = f"{xp:,} / {xp_needed:,} XP"
    draw.text((840 - font_small.getlength(xp_text) if hasattr(font_small, 'getlength') else 750, 120), xp_text, fill=(180, 190, 205, 255), font=font_medium)
    
    level_text = f"LEVEL : {level}"
    draw.text((720, 65), level_text, fill=(0, 229, 255, 255), font=font_large)
...
class Leveling(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if user_data["xp"] >= xp_needed:
            ...
            # توليد وإرسال البطاقة المصورة في الشات مع المنشن
            try:
                card_file = await create_rank_card(message.author, new_level, user_data["xp"], new_level * 200, role_name)
                await message.channel.send(
                    content=f"🎉 مبروك يا {message.author.mention}! لقد صعدت إلى المستوى **{new_level}** 🚀",
                    file=card_file
                )
            except Exception as e:
    ...
    @app_commands.command(name="level", description="معرفة مستواك الحالي وبطاقة الـ XP الخاصة بك")
    ...
    async def level(self, interaction: discord.Interaction, member: discord.Member = None):
        ...
        card_file = await create_rank_card(target, current_level, current_xp, xp_needed, role_name)
